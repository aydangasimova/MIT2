...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        if not (src in self.edges and dest in self.edges):
            raise ValueError('One or both of the suggested nodes are not in graph')
        else:
...
class TestGraph(unittest.TestCase): # A testcase is created by subclassing unittest.TestCase.
# the individual test methods are below
    def setUp(self):
        self.g = Digraph()
        self.na = Node('a')
        self.nb = Node('b')
